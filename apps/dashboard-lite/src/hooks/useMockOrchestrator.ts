...
function generateMockTransaction(): Transaction {
    ...
    const ucpIdSplit = isOnline
        ? `online:en:US:${baseSku}`
        : `local:${storeCode}:${baseSku}`;

    const eventTypes: Transaction["eventType"][] = ["order.completed", "inventory.updated", "loyalty.issued"];
        ...
        const intervalId = setInterval(() => {
            ...
            if (Math.random() < 0.3) {
                setSystemStatus((prev) => {
                    ...
                    const nodes = Object.keys(next[randomCluster]) as any[];
                    const randomNode = nodes[Math.floor(Math.random() * nodes.length)];

                    const states: NodeState[] = ["healthy", "warning", "critical"];
                    // Heavily weight towards returning to healthy
                    const newState = Math.random() > 0.7 ? states[Math.floor(Math.random() * states.length)] : "healthy";

                    (next[randomCluster] as any)[randomNode] = newState;
                    return next;
                });
            }
